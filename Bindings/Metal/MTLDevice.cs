...
public unsafe struct MTLDevice
{
    ...
    public MTLLibrary newLibraryWithSource(string source, MTLCompileOptions options)
    {
        ...
        nint library = IntPtr_objc_msgSend(NativePtr, sel_newLibraryWithSource,
            sourceNSS,
            options,
            out NSError error);
    ...
    {
        nint library = IntPtr_objc_msgSend(NativePtr, sel_newLibraryWithData, data.NativePtr, out NSError error);
    ...
    {
        nint ret = IntPtr_objc_msgSend(NativePtr, sel_newRenderPipelineStateWithDescriptor,
            desc.NativePtr,
            out NSError error);
    ...
    {
        nint ret = IntPtr_objc_msgSend(NativePtr, sel_newComputePipelineStateWithDescriptor,
            descriptor,
            0,
            nint.Zero,
            out NSError error);
        ...
        if (error.NativePtr != nint.Zero)
        {
            throw new Exception("Failed to create new MTLComputePipelineState: " + error.localizedDescription);
        }

        return new MTLComputePipelineState(ret);
    }
    
    public MTLComputePipelineState newComputePipelineStateWithFunction(
        MTLFunction function)
    {
        nint ret = IntPtr_objc_msgSend(NativePtr, sel_newComputePipelineStateWithFunctionr,
            function,
            out NSError error);
    ...
    {
        nint buffer = IntPtr_objc_msgSend(NativePtr, sel_newBufferWithBytes,
            pointer,
            length,
            options);
    ...
    {
        nint buffer = IntPtr_objc_msgSend(NativePtr, sel_newBufferWithLength, length, options);
        return new MTLBuffer(buffer);
    }
    ...
    private static readonly Selector sel_newComputePipelineStateWithFunctionr = "newComputePipelineStateWithFunction:error:";
